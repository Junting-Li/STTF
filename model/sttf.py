...
class STTF(nn.Module):
    ...
    def forward(self,  road_data):
        ...
        road_seq_emb_pos = g_emb[road_traj_pos]+ road_weeks_emb + road_minutes_emb

        road_seq_emb = self.pe(road_seq_emb)
        road_seq_emb_pos = self.pe(road_seq_emb_pos)

        road_type_emb = self.pe(self.type_emb_layer(road_type))

        road_padding_mask = road_traj > 0
        road_padding_mask_pos = road_traj_pos > 0
        ...
        road_seq_emb_pos = self.road_enc(
            src=road_seq_emb_pos,
            type_src=road_type_emb,
            mask=road_padding_mask_pos
        )

        road_seq_emb_pos = self.road_cl_linear(road_seq_emb_pos)
        road_traj_emb_pos = road_seq_emb_pos[:, 0]
        road_e_pos = F.normalize(road_traj_emb_pos, dim=-1)

        logits = torch.matmul(road_e_pos, road_e.T) / self.temp
        ...
        mlm_prediction = mask_road_seq_emb[mask_road_index]
    ...
    def cls_test(self, road_data):
        ...
        fusion_traj_emb = road_seq_emb[:, 0]
