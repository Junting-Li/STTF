...
from dataset.sttf_loader import TrajDataLoader
from model.sttf import STTF
...
class Exp:
    ...
    def test(self):
        ...
        for batch_data in tqdm(self.query_loader, desc='Query     Inference'):
            ...
            road_traj_emb = self.model.test(road_data)
            query_fusion_traj_emb_list.append(road_traj_emb.cpu().numpy())
        ...
        for batch_data in tqdm(self.query_sim_loader, desc='Query Sim Inference'):
            ...
            road_traj_emb = self.model.test(road_data)
            query_fusion_sim_traj_emb_list.append(road_traj_emb .cpu().numpy())
        ...
        for batch_data in tqdm(self.database_loader, desc='Database  Inference'):
            ...
            road_traj_emb = self.model.test(road_data)
            database_fusion_traj_emb_list.append(road_traj_emb.cpu().numpy())
